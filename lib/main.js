...
function compareAndPublish(headCoverage, parsedCoverage) {
    var finalResult = {}
    Object.keys(parsedCoverage).forEach(function(component) {
        if(headCoverage[component]) {
            finalResult[component] = parseFloat(parsedCoverage[component]) > parseFloat(headCoverage[component]) ? true : false
        }
    })
...
function checkAndPublishCoverage(parsedResult) {
    axios.get('https://kvdb.io/' + process.env.KVDB_SECRET_KEY + '/app').then(function(response) {
        ...
        compareAndPublish(response.data, parsedResult)
    })
}
